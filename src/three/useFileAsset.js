...
function handleAsset(fn, cache, args, lifespan = 0, preload = false) {
	for (const entry of cache) {
		// Find a match
		if (deepEqual([args[0], args[1].path], entry.args)) {
	...
	const entry = {
		...
		promise: fn(...args) // Response can't be undefined or else the loop above wouldn't be able to return it
			// This is for promises that do not return results (delays for instance)
			.then((response) => (entry.response = response != null ? response : true))
			.catch((e) => (entry.error = e != null ? e : "unknown error"))
			.then(() => {
				if (lifespan > 0) {
					setTimeout(() => {
...
function createAsset(fn, lifespan = 0) {
	...
	return {
		...
		peek: (...args) => {
			...
			return (_cache$find = cache.find((entry) => deepEqual(args, entry.args))) == null
				? void 0
				: _cache$find.response;
		},
	};
}

function useFileAsset(fn, ...args) {
	return handleAsset(fn, globalCache, args, useFileAsset.lifespan);
}

useFileAsset.lifespan = 0;

useFileAsset.clear = (...args) => clear(globalCache, ...args);

useFileAsset.preload = (fn, ...args) => void handleAsset(fn, globalCache, args, useFileAsset.lifespan, true);

useFileAsset.peek = (...args) => {
	var _globalCache$find;

	return (_globalCache$find = globalCache.find((entry) => deepEqual(args, entry.args))) == null
		? void 0
		: _globalCache$find.response;
};

export { createAsset, useFileAsset };
